...
import type { Explain } from '@/lib/types/index'
...
export default function ExplainModal() {
  ...
  // Ensure the array is typed so map() callback params aren't implicit any
  const items: Explain[] = Array.isArray(state.explainRow.explain)
    ? (state.explainRow.explain as Explain[])
    : [];
        ...
        <div style={{ padding: 16 }}>
          {items.length > 0 ? (
            <table style={{ width: '100%', borderCollapse: 'collapse' }}>
              ...
              <tbody>
                {items.map((e: Explain, idx: number) => (
