...
type Row = {
  ...
  sector?: string;
  volume?: number;
  priceChangePct?: number; // new, supplied by backend
};
...
export default function Page() {
  ...
  function toggleSort(key: SortKey) {
    if (sortBy === key) setSortDir(sortDir === 'asc' ? 'desc' : 'asc');
      ...
      setSortBy(key);
      setSortDir(
        key === 'symbol' || key === 'companyName' || key === 'sector'
          ? 'asc'
          : 'desc'
      );
    }
  ...
  const priceColTitle = priceChangeDays
    ? `Price (${priceChangeDays} days %change)`
    : 'Price';
  ...
  return (
    <main style={{ maxWidth: 1100, margin: '40px auto', padding: 16 }}>
      ...
      <p style={{ color: '#334155', marginBottom: 16 }}>
        Set filters → <b>Run</b>. Click column headers to sort. ({sorted.length}{' '}
        results)
      </p>
      ...
      >
        ...
        {/* Market Cap inputs */}
        <div>
          <div style={{ fontSize: 12, color: '#64748b' }}>Market Cap Min</div>
        ...
        <div>
          <div style={{ fontSize: 12, color: '#64748b' }}>Market Cap Max</div>
          <div style={{ display: 'flex', alignItems: 'center', gap: 6 }}>
            <span>$</span>
            <input
              value={mktMax}
